...
class ViewController: UIViewController {
  ...
  var presentedController: SecondViewController?
  
  override func viewDidAppear(_ animated: Bool) {
    super.viewDidAppear(animated)
    canPresent = true
    
    presentedController = SecondViewController(nibName: nil, bundle: nil)
    presentedController?.willMove(toParent: self)
    presentedController?.view.backgroundColor = UIColor.yellow
    self.addChild(presentedController!)
    var frame = self.view.bounds.insetBy(dx: 25, dy: 100)
    frame.origin.y -= 50
    presentedController?.view.frame = frame
    self.view.addSubview(presentedController!.view)
    presentedController?.didMove(toParent: self)
